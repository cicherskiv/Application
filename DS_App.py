...
def generate_and_preprocess_data(params, replication_seed, run='train'):
    ...
    input_stage1 = O1_filtered.t()
    params['input_dim_stage1'] = input_stage1.shape[1] #  (H_1)  
    print("in gen", params['input_dim_stage1'], input_stage1.shape[1])
...
def simulations(V_replications,fixed_config, params, config_number):
    ...
    paramsT_DQL = copy.deepcopy(params)
    paramsT_DS = copy.deepcopy(params)
    
    paramsT_DS['f_model'] = 'surr_opt'
    paramsT_DQL['f_model'] = 'DQlearning'
    paramsT_DQL['num_networks'] = 1  



    #fixed_config['optimizer_type'] = params['optimizer_type']
    #fixed_config['n_epoch'] = params['n_epoch']
    fixed_config['num_layers'] = params['num_layers']
    fixed_config['activation_function'] = params['activation_function']
    fixed_config['hidden_dim_stage1'] = params['hidden_dim_stage1']
    fixed_config['hidden_dim_stage2'] = params['hidden_dim_stage2']
    #fixed_config['batch_size'] = params['batch_size']
    #fixed_config['optimizer_lr'] = params['optimizer_lr']

    paramsE_DQL = copy.deepcopy(fixed_config)
    paramsE_DS = copy.deepcopy(fixed_config)
    
    paramsE_DS['f_model'] = 'surr_opt'
    paramsE_DQL['f_model'] = 'DQlearning'
    paramsE_DQL['num_networks'] = 1  
    ...
    for replication in tqdm(range(params['num_replications']), desc="Replications_M1"):
        ...
        if params.get('run_DQlearning', True):
            ...
            paramsT_DQL['input_dim_stage1'] = params['input_dim_stage1'] + 1 # Ex. TAO: 5 + 1 = 6 # (H_1, A_1)
            paramsT_DQL['input_dim_stage2'] = params['input_dim_stage2'] + 1 # Ex. TAO: 7 + 1 = 8 # (H_2, A_2)

            paramsE_DQL['input_dim_stage1'] = params['input_dim_stage1'] + 1 # Ex. TAO: 5 + 1 = 6 # (H_1, A_1)
            paramsE_DQL['input_dim_stage2'] = params['input_dim_stage2'] + 1 # Ex. TAO: 7 + 1 = 8 # (H_2, A_2)

            start_time = time.time()  # Start time recording
            trn_val_loss_tpl_DQL = DQlearning(tuple_train, tuple_val, paramsE_DQL, config_number)
            end_time = time.time()  # End time recording
        ...
        if params.get('run_surr_opt', True):
            ...
            paramsT_DS['input_dim_stage1'] = params['input_dim_stage1']  # Ex. TAO: 5  # (H_1, A_1)
            paramsT_DS['input_dim_stage2'] = params['input_dim_stage2']  # Ex. TAO: 7  # (H_2, A_2)
            paramsE_DS['input_dim_stage1'] = params['input_dim_stage1']  # Ex. TAO: 5  # (H_1, A_1)
            paramsE_DS['input_dim_stage2'] = params['input_dim_stage2']  # Ex. TAO: 7  # (H_2, A_2)

            start_time = time.time()  # Start time recording
            trn_val_loss_tpl_DS, epoch_num_model_DS = surr_opt(tuple_train, tuple_val, paramsE_DS, config_number)
            end_time = time.time()  # End time recording
        ...
        print(paramsT_DQL)
        start_time = time.time()  # Start time recording
        V_replications, df_DQL, df_DS, df_Tao = eval_DTR(V_replications, replication, df_DQL, df_DS, df_Tao, paramsT_DQL, paramsT_DS, config_number)
        end_time = time.time()  # End time recording
...
def run_training(config, config_updates, fixed_config, V_replications, config_number, replication_seed):
    torch.manual_seed(replication_seed)
    ...
    V_replications, df_DQL, df_DS, df_Tao, losses_dict, epoch_num_model_lst = simulations(V_replications,fixed_config, local_config, config_number)
...
def run_training_with_params(params):
    ...
    config, current_config, fixed_config, V_replications, i, config_number = params
    return run_training(config, current_config, fixed_config, V_replications, config_number, replication_seed=i)
 

def run_grid_search(config,fixed_config, param_grid):
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        ...
        for config_number, current_config in enumerate(param_combinations):
            for i in range(grid_replications): 
                ...
                params = (config, current_config,fixed_config, V_replications, i, config_number)
    ...
    load_and_process_data(config, fixed_config, folder)
...
def main():
    ...
    param_grid = {
        ...
        'num_layers': [1], # 1,2,3,4,5,6,7
        'n_epoch':[150],
        'optimizer_type': ['adam']
 
    }
    # Perform operations whose output should go to the file
    run_grid_search(config, fixed_config, param_grid)
